...
public class Advent9_2 {
    ...
    private Integer getNextvalue(ArrayList<Integer> series){
        ...
        firsts.add(series.get(0));

        ArrayList<Integer> diffSeries = getDifferences(series);
        firsts.add(diffSeries.get(0));
        ...
        System.out.println("End Values:" + seeRow(firsts));

        nexts.add(0);

        for(int i = firsts.size()-2; i >=0; i--){
            nexts.add(firsts.get(i) - nexts.get(nexts.size()-1));
        }
